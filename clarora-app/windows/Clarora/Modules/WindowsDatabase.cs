...
namespace Clarora
{
    ...
    public sealed class WindowsDatabase : IDisposable
    {
        ...
        [DllImport("winsqlite3", CallingConvention = CallingConvention.Cdecl)] private static extern int sqlite3_open_v2(byte[] path, out IntPtr handle, int flags, IntPtr vfs);
        [DllImport("winsqlite3", CallingConvention = CallingConvention.Cdecl)] private static extern int sqlite3_stmt_readonly(IntPtr statement);

        private void Check(int code) { if (code != 0) throw new InvalidOperationException(Marshal.PtrToStringUni(sqlite3_errmsg16(db))); }
        // Independent read-only handle: never reuse the learning database connection.
        [ReactMethod("querySnapshot")]
        public Task<string> QuerySnapshot(string path, string sql) => Task.Run(() => {
            IntPtr handle = IntPtr.Zero, statement = IntPtr.Zero;
            try {
                var utf8Path = System.Text.Encoding.UTF8.GetBytes(WindowsFiles.PathOf(path) + "\0");
                if (sqlite3_open_v2(utf8Path, out handle, 1 /* SQLITE_OPEN_READONLY */, IntPtr.Zero) != 0)
                    throw new InvalidOperationException("无法只读打开 Anki 数据库");
                if (sqlite3_prepare16_v2(handle, sql, -1, out statement, IntPtr.Zero) != 0 ||
                    statement == IntPtr.Zero || sqlite3_stmt_readonly(statement) == 0)
                    throw new InvalidOperationException("无法读取 Anki 数据库，请确认导出时启用了支持旧版本");
                var rows = new Windows.Data.Json.JsonArray();
                int status;
                while ((status = sqlite3_step(statement)) == Row) {
                    var row = new Windows.Data.Json.JsonObject();
                    for (int column = 0; column < sqlite3_column_count(statement); column++) {
                        ...
                        var value = Marshal.PtrToStringUni(sqlite3_column_text16(statement, column)) ?? "";
                        row[name] = Windows.Data.Json.JsonValue.CreateStringValue(value);
                    }
                    rows.Add(row);
                }
                if (status != Done) throw new InvalidOperationException("读取 Anki 数据库失败");
