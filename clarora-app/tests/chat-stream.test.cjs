...
function setup() {
  ...
  }).outputText, { exports, XMLHttpRequest: Request, require: name => name === 'react-native' ? { Platform: { OS: 'macos' } } : name === '../data/database' ? { getSetting: async () => JSON.stringify({ baseUrl: 'http://localhost:8080/v1', model: 'local-model' }) } : name === './secrets' ? { secretStore: () => null, isDevBuild: () => false } : {}, setTimeout, clearTimeout });
  const events = [];
